...
const appVersion = (config.public as Record<string, unknown>).appVersion as string || '0.4'
...
<template>
  <header
    class="flex items-center justify-between px-3 sm:px-5 h-12 sm:h-14 border-b"
    :style="{ background: 'var(--bg-1)', borderColor: 'var(--line)' }"
  >
    <!-- Logo -->
    <div class="flex items-center gap-2 sm:gap-3">
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" class="text-[var(--accent)] shrink-0">
        <path
      ...
      <span class="font-display text-lg sm:text-xl not-italic" style="color: var(--ink-0)">Star Watcher</span>
      <span
        class="hidden md:inline font-mono text-[10px] px-2 py-0.5 rounded-full"
        :style="{ background: 'var(--bg-3)', color: 'var(--ink-3)' }"
      >
        v{{ appVersion }} · orange pi
      </span>
    </div>
    ...
    <!-- Status (right) -->
    <div class="flex items-center gap-2 sm:gap-3">
      <div class="flex items-center gap-1.5 sm:gap-2 font-mono text-[11px] sm:text-xs" style="color: var(--ink-2)">
        <span
          class="size-2 rounded-full shrink-0"
          :style="{ background: isConnected ? 'var(--good)' : 'var(--bad)' }"
        />
        <span class="truncate max-w-[80px] sm:max-w-none">{{ cameraModel }}</span>
        <span v-if="isConnected" class="hidden sm:inline" style="color: var(--ink-3)">· online</span>
        <span v-else class="hidden sm:inline" style="color: var(--bad)">· offline</span>
      </div>
      <button
        class="p-1.5 rounded"
